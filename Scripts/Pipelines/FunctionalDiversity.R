...
#' @param start_col_samples Number of the col corresponding to the first sample in the tab_eDNA dataframe (avoid confusion when a different number of classification level is provided)
...
#' Default : F
#' @param axes4alpha number of axes used for computing alpha-diversity indices 
#' (WARNING:Number of species should strictly be higher than the number of axes to compute the convex hull for Fric in the mFD function)
#' Default : c("PC1","PC2","PC3","PC4","PC5")
...
#' 
#' @author Romane Rozanski (rrozanski@ethz.ch), Camille Albouy (calbouy@ethz.ch), David Eme (david.eme@inrae.fr)
...
#'          much information)
#'          
#'@note Updated : 2022/06/30 by Romane Rozanski. Corrections :
#'       - adding parameter "axes4alpha" to get the possibility to choose how many PCOA axes are wanted 
#'       - some minor corrections 
#'       
#'@note Updated : 2022/11/16 by Romane Rozanski. Corrections :
#'       - adding alpha indice VNFD (variance of nearest functional distance;correspond to var of "fnnd")
#'       
#'@note Updated : 2022/02/06 by Romane Rozanski. Corrections :
#'       - adding input start_col_samples (number of the col containing the 1st sample): avoid confusion when a different number of classification level is provided
#'       
functional_diversity <- function(tab_eDNA,sp_tr_all_sp, sp_tr_cat,
                                 Classif=Classif_end,start_col_samples,
                                 abundance=FALSE,axes4alpha=c("PC1","PC2","PC3","PC4","PC5"),axes4beta=c(1:2),
  ...
  tab_nb_1 <- tab_eDNA[,start_col_samples:ncol(tab_eDNA)] #delete all the useless columns of classification
  ...
  list_100_alpha_fd_indices <- lapply(seq(1, length(list_100_sp_faxes_coord)), function(x){
    mFD::alpha.fd.multidim(list_100_sp_faxes_coord[[x]][, axes4alpha], asb_sp_w,
  ...
  ###Var of Nearest Functional Pairwise Distance calculation (variance of "fnnd" David Eme; see source function) : regularity indice
  list_100_VNFD<-list()
  for (x in 1:length(list_100_sp_dist)){
    list_100_VNFD[[x]]<-data.frame(VNFD=Var.mNFD(list_100_sp_dist[[x]], asb_sp_w))
  }
  
  
  #VNFD mean
  mean_VNFD<-do.call(rbind, lapply(seq(1, length(sites_list)), function(x){
    DFtemp = do.call(rbind, lapply(seq(1, length(list_100_VNFD)), function(i){
      list_100_VNFD[[i]][rownames(list_100_VNFD[[i]])== sites_list[x],]
    }))
    c(Mean=apply(DFtemp, 2, mean, na.rm=T),Sd=apply(DFtemp, 2, sd, na.rm=T))
  }))
  rownames(mean_VNFD)<-sites_list
  colnames(mean_VNFD)<-c("Mean.VNFD","Sd.VNFD")
  ...
  average_all_alpha_fd_indices <- cbind(average_alpha_fd_indices,mean_VFPD, mean_VNFD,alpha.hill_tot)
  colnames(average_all_alpha_fd_indices)[length(average_all_alpha_fd_indices)]<-"sd.FD_q0"
