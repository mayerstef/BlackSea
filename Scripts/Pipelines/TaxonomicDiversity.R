...
#' 
#' @note  updated 2022/03/30 (Pierre Veron) : added the condition has.replicates
#' @note  updated 2022/06/30 (Romane Rozanski) : some minor correction


taxonomic_diversity<-function(tab_eDNA, abundance=FALSE, has.replicates = F){
  ...
  if (abundance==TRUE){
    shannon_ind<-diversity(asb_sp_w, index="shannon", base=exp(1)) #shannon
    simpson_ind<-diversity(asb_sp_w, index="simpson") #simpson indice
