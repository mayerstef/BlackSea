...
#'       
#'@note Updated : 2022/30/06 by Romane Rozanski. Corrections : 
#'       - minor bugs
#'       
...
phylo_diversity<-function(trees = trees_acti_chond, tab_eDNA, savepathindic,  abundance=FALSE,
                          has.replicates = F){
  nb_rd <- length(trees)
  ...
  tab_nb_1 <- tab_eDNA[order(rownames(tab_eDNA)), ] #alphabetic order
  ...
  #check<- asb_sp_w[,which(!colnames(asb_sp_w) %in%  data_acti_chond_trees[[1]]$tip.label)] #to check if some names of species are not in the tree
  ...
  if (abundance==FALSE){
    ...
    list_beta_indices<-lapply(seq(1,length(data_acti_chond_trees)),function(i){
    ...
    list_beta_MPFD<- lapply(seq(1, length(list_sp_dist)), function(x){
    ...
    list_beta_MNND<-lapply(seq(1, length(list_sp_dist)),function(x){
    ...
    rownames(beta_indice_phylo)<-str_c(Mean_bjac$Var1,paste="_",Mean_bjac$Var2) 
  ...
  rownames(Hill_beta) <- str_c(Hill_beta$site2, paste="_", Hill_beta$site1)  #to get the same order as beta_indices_phylo
  Hill_beta[,1:2]<-NULL
